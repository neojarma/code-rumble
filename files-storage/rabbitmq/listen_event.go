...
func ListenEvent(ctx context.Context, queueName string, function func(*entity.Question)) error {
...
func doTask(msgs <-chan amqp.Delivery, function func(*entity.Question)) error {
	log.Println("ready to listen event")
	for v := range msgs {
		payload := new(entity.Question)
